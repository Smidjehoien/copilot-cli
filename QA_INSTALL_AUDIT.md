...
- [x] Keep retries/timeouts configurable via env vars for enterprise networks (implemented via `CURL_*` and `WGET_*`).
...
- [x] Document fallback behavior and known failure modes in README (proxy, TLS intercept, winget missing).
...
- Added a short “Troubleshooting install script” section in README covering:
